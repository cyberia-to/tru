...
[[neurons]] lock [[balance]] to create [[will]]. [[will]] auto-distributes across [[cyberlinks]], producing [[attention]] at target [[particles]]. the [[tri-kernel]] aggregates all [[attention]] into a single [[probability]] distribution φ* over all [[particles]]. this distribution is focus
...
the transition matrix P is stochastic, irreducible, aperiodic. by [[Perron-Frobenius theorem]], a unique φ* exists:

$$\phi^* P = \phi^*, \quad \sum_i \phi^*_i = 1, \quad \phi^*_i > 0 \;\forall\, i$$

convergence rate determined by spectral gap: $\|\phi^{(t)} - \phi^*\| \leq C \cdot (1-\lambda)^t$
