...
impl DialectAdj {
    fn build(edges: &[Edge], assign: &[usize], head: usize, n: usize, maxw: u128) -> Self {
        let mut acc: Vec<std::collections::HashMap<u32, i128>> = vec![std::collections::HashMap::new(); n];
        ...
        let out = acc
            .into_iter()
            .map(|row| row.into_iter().map(|(j, w)| (j, Fx::ratio_u128(w as u128, maxw))).collect())
            .collect();
        DialectAdj { n, out }
    }
...
impl Embed<'_> {
    ...
    fn t_apply(&self, y: &[Fx]) -> Vec<Fx> {
        (0..self.d).map(|c| (0..self.n).fold(Fx::ZERO, |a, i| a + self.data[i * self.d + c] * y[i])).collect()
    }
}
...
fn project(a: &DialectAdj, e: &Embed, l_eff: usize) -> Vec<Vec<Fx>> {
    ...
    let cols: Vec<Vec<Fx>> = (0..e.d).map(|c| e.t_apply(&a.apply_pow(&e.col(c), l_eff))).collect();
    // cols[c] is column c of P; transpose to row-major P[i][j].
    (0..e.d).map(|i| (0..e.d).map(|j| cols[j][i]).collect()).collect()
}
...
fn dense_tensor(name: String, m: &[Vec<Fx>]) -> Tensor {
    ...
    Tensor { name, shape: vec![d as u64, d as u64], encoding: Encoding::U16, data }
}
...
pub fn attention(
    ...
    let d_h = if h == 0 { d } else { d / h };
    let maxw = edges.iter().filter(|e| e.stake > 0).map(|e| e.stake as u128).max().unwrap_or(1).max(1);

    // One dialect adjacency per head (index into dialects.set).
    let dialect_adj: Vec<DialectAdj> = (0..h).map(|head| DialectAdj::build(edges, &dialects.assign, head, n, maxw)).collect();
    ...
    for layer in 0..l {
        ...
        for (head, a) in dialect_adj.iter().enumerate() {
            ...
            for c in 0..d_h {
                let ss = if c < svd.sigma.len() { svd.sigma[c].sqrt() } else { Fx::ZERO };
                for i in 0..d {
                    if c < svd.u.len() {
        ...
        tensors.push(dense_tensor(format!("model.layers.{layer}.self_attn.q_proj.weight"), &q));
        tensors.push(dense_tensor(format!("model.layers.{layer}.self_attn.k_proj.weight"), &k));
        tensors.push(dense_tensor(format!("model.layers.{layer}.self_attn.v_proj.weight"), &v));
        tensors.push(dense_tensor(format!("model.layers.{layer}.self_attn.o_proj.weight"), &o));
...
mod tests {
    ...
    fn link(from: u8, to: u8) -> Cyberlink {
        Cyberlink { neuron: hash(from), from: hash(from), to: hash(to), token: 0, amount: 100, valence: 1, block: 0 }
    }
    ...
    fn emits_five_tensors_per_layer_with_right_shapes() {
        ...
        let q0 = t.iter().find(|t| t.name == "model.layers.0.self_attn.q_proj.weight").unwrap();
        assert_eq!(q0.shape.len(), 2);
        assert_eq!(q0.data.len() as u64, q0.shape[0] * q0.shape[1]);
        let ab = t.iter().find(|t| t.name == "model.layers.0.self_attn.alpha_beta.weight").unwrap();
