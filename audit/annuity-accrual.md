...
- a single epoch's contribution is exactly `ω·Δφ*` (the integrand).
...
- a falsified link (`ω → 0`) stops drawing from that epoch on but the
  running total never drops — past accrual is never reversed, per §12.
